...
def json_convert():
    ...
    new_list = [ { k : v.strftime("%Y-%m-%d") if k == "ctime" else v for k, v in e.items() } for e in data_list ] 
...
def json_eval():
    ...
    data = '{"content":"this is a test demo", "name": "alex", "genger": False, "sort": (145,675,857,678)}'
    resolved = eval(data)
    print(resolved)
    print(resolved['sort'])
